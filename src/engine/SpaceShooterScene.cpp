...
SpaceShooterScene::SpaceShooterScene(SSD1963& display)
    : Scene(display)
    , m_player_x(0), m_player_y(0), m_player_dir(1)
    , m_lives(3), m_score(0)
    , m_timer(0), m_shoot_timer(0), m_spawn_timer(0), m_move_timer(0)
    , m_sprites_loaded(false), m_planet_drawn(false)
{
...
bool SpaceShooterScene::on_enter() {
    ...
    m_display.clear_screen(rgb(5, 5, 15));
...
void SpaceShooterScene::draw() {
    ...
    // Planets (static background, drawn once)
    if (!m_planet_drawn) {
        m_planet_drawn = true;
